function getDimensionFinalMultiplier(tier) {
  ...
  if (player.currentEternityChall == "eterc11")
    return player.infinityPower
      ...
      .max(1)
      .times(
        getDimensionBoostPower()
          .pow(player.resets - tier + 1)
          .max(1)
      );
  ...
  if (player.infinityUpgrades.includes("totalMult"))
    multiplier = multiplier.times(totalMult);
  if (player.infinityUpgrades.includes("currentMult"))
    multiplier = multiplier.times(currentMult);
  ...
  if (tier == 8 && player.achievements.includes("r23"))
    multiplier = multiplier.times(1.1);
  else if (player.achievements.includes("r34"))
    multiplier = multiplier.times(1.02);
  if (tier <= 4 && player.achievements.includes("r43"))
  ...
  if (player.achievements.includes("r74") && player.currentChallenge != "")
    multiplier = multiplier.times(1.4);
  if (player.achievements.includes("r77"))
    multiplier = multiplier.times(1 + tier / 100);
  if (player.achievements.includes("r56") && player.thisInfinityTime < 1800)
    multiplier = multiplier.times(3600 / (player.thisInfinityTime + 1800));
  if (player.achievements.includes("r78") && player.thisInfinityTime < 3)
    multiplier = multiplier.times(3.3 / (player.thisInfinityTime + 0.3));
  if (
    ...
    multiplier = multiplier.times(
      Math.max(2400 / (player.thisInfinityTime + 600), 1)
    );
  if (player.achievements.includes("r91") && player.thisInfinityTime < 50)
    multiplier = multiplier.times(
      Math.max(301 - player.thisInfinityTime * 6, 1)
    );
  if (player.achievements.includes("r92") && player.thisInfinityTime < 600)
    multiplier = multiplier.times(
      Math.max(101 - player.thisInfinityTime / 6, 1)
    );
  if (player.achievements.includes("r84"))
    multiplier = multiplier.times(player.money.pow(0.00004).plus(1));
  else if (player.achievements.includes("r73"))
    multiplier = multiplier.times(player.money.pow(0.00002).plus(1));

  if (player.timestudy.studies.includes(71) && tier !== 8)
    multiplier = multiplier.times(
      calcTotalSacrificeBoost()
        .pow(0.25)
        .min("1e210000")
    );
  if (player.timestudy.studies.includes(91))
    multiplier = multiplier.times(
      Decimal.pow(10, Math.min(player.thisEternity, 18000) / 60)
    );
  if (player.timestudy.studies.includes(101))
    multiplier = multiplier.times(Decimal.max(player.replicanti.amount, 1));
  if (player.timestudy.studies.includes(161))
    multiplier = multiplier.times(new Decimal("1e616"));
  if (player.timestudy.studies.includes(234) && tier == 1)
    multiplier = multiplier.times(calcTotalSacrificeBoost());

  multiplier = multiplier.times(player.postC3Reward);
  if (player.challenges.includes("postc8") && tier < 8 && tier > 1)
    multiplier = multiplier.times(mult18);

  if (player.currentChallenge == "postc6")
    multiplier = multiplier.dividedBy(player.matter.max(1));
  if (player.currentChallenge == "postc8")
    multiplier = multiplier.times(postc8Mult);

  if (player.currentChallenge == "postc4" && player.postC4Tier != tier)
    multiplier = multiplier.pow(0.25);
...
function getDimensionDescription(tier) {
  ...
  let description =
    shortenDimensions(player[name + "Amount"]) + " (" + dimBought(tier) + ")";
  if (tier == 8)
    description =
      formatInfOrEter(Math.round(player[name + "Amount"])) +
      " (" +
      dimBought(tier) +
      ")";
...
function multiplySameCosts(cost) {
  ...
  for (let i = 1; i <= 8; ++i) {
    if (player[tiers[i] + "Cost"].e == cost.e)
      player[tiers[i] + "Cost"] = player[tiers[i] + "Cost"].times(tierCosts[i]);
  }
  if (player.tickSpeedCost.e == cost.e)
    player.tickSpeedCost = player.tickSpeedCost.times(
      player.tickspeedMultiplier
    );
}
...
function multiplyPC5Costs(cost, tier) {
  ...
  if (tier < 5) {
    for (var i = 1; i < 9; i++) {
      if (player[tiers[i] + "Cost"].e <= cost.e) {
        player[tiers[i] + "Cost"] = player[tiers[i] + "Cost"].times(
          player.costMultipliers[i - 1]
        );
        if (player[tiers[i] + "Cost"].gte(Number.MAX_VALUE))
          player.costMultipliers[i - 1] = player.costMultipliers[i - 1].times(
            10
          );
      }
    }
...
function canBuyDimension(tier) {
  ...
  if (
    tier > 1 &&
    player[TIER_NAMES[tier - 1] + "Amount"] == 0 &&
    player.eternities < 30
  )
    return false;
  if (
...
function getDimensionPowerMultiplier(tier) {
  ...
  if (
    ...
    dimMult = Math.pow(10 / 0.3, Math.random()) * 0.3;
  ...
  if (player.timestudy.studies.includes("242"))
    dimMult += Math.pow(player.replicanti.galaxies / 200, 0.25);
  return dimMult;
}
...
function buyOneDimension(tier) {
  ...
  if (dimBought(tier) === 0) {
    ...
    if (player[name + "Cost"].gte(Number.MAX_VALUE))
      player.costMultipliers[tier - 1] = player.costMultipliers[tier - 1].times(
        player.dimensionMultDecrease
      );
    floatText(
      name + "D",
      "x" + shortenMoney(getDimensionPowerMultiplier(tier))
    );
  }
  ...
  if (
    ...
    player.chall2Pow = 0;
  if (
    player.currentChallenge == "challenge8" ||
    player.currentChallenge == "postc1"
  )
  ...
  if (
    ...
    if (
      ...
      if (player[name + "Cost"].lt(Number.MAX_VALUE)) {
        let failsafe = 0;
        while (
          player.money.gt(player[name + "Cost"].times(10)) &&
          x > 0 &&
      ...
      if (player[name + "Cost"].gte(Number.MAX_VALUE)) {
        ...
        var buying =
          Math.floor((Math.sqrt(Math.pow(b, 2) - c * a * 4) - b) / (2 * a)) + 1;
...
function canAfford(cost) {
  return (
    ((cost.lt(new Decimal("1.79e308")) && !player.break) || player.break) &&
    cost.lte(player.money)
  );
}
...
function timeMult() {
  ...
  if (!player.achievements.includes("r151")) {
    if (player.infinityUpgrades.includes("timeMult"))
      mult = mult.times(Math.pow(player.totalTimePlayed / 1200, 0.15));
    if (player.infinityUpgrades.includes("timeMult2"))
      mult = mult.times(
        Decimal.max(Math.pow(player.thisInfinityTime / 2400, 0.25), 1)
      );
    if (player.achievements.includes("r76"))
      mult = mult.times(
        Math.pow(player.totalTimePlayed / (600 * 60 * 48), 0.05)
      );
  } else if (player.achievements.includes("r151")) {
    if (player.infinityUpgrades.includes("timeMult"))
      mult = mult.times(timeMultUpg(1, 1));
    if (player.infinityUpgrades.includes("timeMult2"))
      mult = mult.times(timeMultUpg(2, 1));
    if (player.achievements.includes("r76"))
      mult = mult.times(timeMultUpg(3, 1));
...
function getDimensionProductionPerSecond(tier) {
  let ret = Decimal.floor(player[TIER_NAMES[tier] + "Amount"])
    .times(getDimensionFinalMultiplier(tier))
    .times(1000)
    .dividedBy(player.tickspeed);
  if (player.currentChallenge == "challenge7") {
    if (tier == 4)
      ret = player[TIER_NAMES[tier] + "Amount"]
        .floor()
        .pow(1.3)
        .times(getDimensionFinalMultiplier(tier))
