...
function updateDimensions() {
    ...
    if (document.getElementById("infinity").style.display == "block") {
        if (document.getElementById("preinf").style.display == "block") {
            ...
            document.getElementById("postinfi11").innerHTML = "All dimension multipliers increase based on total antimatter produced<br>Currently: "+ Math.pow(player.totalmoney.e+1, 0.5).toFixed(2)+"x<br>Cost: "+shortenCosts(1e4)+" IP"
            document.getElementById("postinfi21").innerHTML = "All dimension multipliers increase based on current antimatter<br>Currently: "+ Math.pow(player.money.e+1, 0.5).toFixed(2)+"x<br>Cost: "+shortenCosts(5e4)+" IP"
            ...
            document.getElementById("postinfi22").innerHTML = "All dimension multipliers increase based on achievements completed <br>Currently: "+achievementMult.toFixed(2)+"x<br>Cost: "+shortenCosts(1e6)+" IP"
            document.getElementById("postinfi12").innerHTML = "All dimension multipliers increase based on infinitied stat <br>Currently: "+(1+Math.log10(getInfinitied()+1)*10).toFixed(2)+"x<br>Cost: "+shortenCosts(1e5)+" IP"
            if (player.timestudy.studies.includes(31)) document.getElementById("postinfi12").innerHTML = "All dimension multipliers increase based on infinitied stat <br>Currently: "+shortenMoney(Math.pow((Math.log10(getInfinitied()+1)*10).toFixed(2), 4))+"x<br>Cost: "+shortenCosts(1e5)+" IP"
            document.getElementById("postinfi41").innerHTML = "Galaxies are 50% stronger.<br>Cost: "+shortenCosts(5e11)+" IP"
            document.getElementById("postinfi32").innerHTML = "All dimension multipliers increase based on slowest challenge run<br>Currently:"+Decimal.max(10*3000/worstChallengeTime, 1).toFixed(2)+"x<br>Cost: "+shortenCosts(1e7)+" IP"
            ...
            document.getElementById("postinfi23").innerHTML = "Unlock the option to bulk buy Dimension Boosts <br>Cost: "+shortenCosts(5e9)+" IP"
    ...
    if (document.getElementById("eternityupgrades").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
        ...
        document.getElementById("eter2").innerHTML = "Infinity Dimensions multiplier based on eternities ((x/200)^log4(2x))<br>Currently: "+shortenMoney(eterUpg2Mult())+"x<br>Cost: 10 EP"
...
function updateInfCosts() {
    ...
    if (document.getElementById("replicantis").style.display == "block" && document.getElementById("infinity").style.display == "block") {
        ...
        if (player.timestudy.studies.includes(225)) extraGals += Math.floor(player.replicanti.amount.e / 1000)
        if (player.timestudy.studies.includes(226)) extraGals += Math.floor(player.replicanti.gal / 15)
        if (extraGals !== 0) document.getElementById("replicantireset").innerHTML = "Reset replicanti amount for a free galaxy.<br>"+player.replicanti.galaxies + "+"+extraGals+ " replicated galaxies created."
        else document.getElementById("replicantireset").innerHTML = (player.replicanti.galaxies !== 1) ? "Reset replicanti amount for a free galaxy.<br>"+player.replicanti.galaxies + " replicated galaxies created." : "Reset replicanti amount, but get a free galaxy<br>"+player.replicanti.galaxies + " replicated galaxy created."
    ...
    if (document.getElementById("timestudies").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
        ...
        document.getElementById("82desc").textContent = "Currently: "+shortenMoney(Decimal.pow(1.0000109,Math.pow(player.resets,2)).min('1e80000'))+"x"
        ...
        if (player.etercreq !== 1) document.getElementById("ec1unl").innerHTML = "Eternity Challenge 1<span>Requirement: "+(ECTimesCompleted("eterc1")+1)*200+" Eternities<span>Cost: 30 Time Theorems"
...
document.getElementById("importbtn").onclick = function () {
    ...
    if (sha512_256(save_data) === "de24687ee7ba1acd8f5dc8f71d41a3d4b7f14432fff53a4d4166e7eea48a88c0") {
        ...
        player.options.secretThemeKey = save_data;
        setTheme(player.options.theme);
    } else if (sha512_256(save_data) === "c0a9d1097cae8247796ab13c493276918e3ee0369608e59e6ed5fd5f9d0ca336") {
        player = metaplayer;
        saved = 0;
        totalMult = 1
        currentMult = 1
...
function eternity(force, auto) {
    if ((player.infinityPoints.gte(Number.MAX_VALUE) && (!player.options.eternityconfirm || auto || confirm("Eternity will reset everything except achievements and challenge records. You will also gain an Eternity point and unlock various upgrades."))) || force === true) {
        ...
        if (player.dilation.active && (!force || player.infinityPoints.gte(Number.MAX_VALUE))) {
            let tachyonGain = Math.max(getDilGain() - player.dilation.totalTachyonParticles, 0)
        ...
        player = {
            ...
            timeDimension8: player.timeDimension8,
            eternityPoints: player.eternityPoints,
            eternities: player.eternities+(player.dilation.upgrades.includes(12) ? Math.floor(Decimal.pow(player.dilation.dilatedTime, .1).toNumber()): 1),
            thisEternity: 0,
            bestEternity: player.bestEternity,
            eternityUpgrades: player.eternityUpgrades,
            epmult: player.epmult,
...
function canUnlockEC(idx, cost, study, study2) {
    ...
    switch(idx) {
        case 1:
        if (player.eternities >= 200+(ECTimesCompleted("eterc1")*200)) return true
        break;
...
let getDilRebuyableUpgCost = function (i) {
  ...
  if (i === 4 && cost.gte(9e99)) {
    cost = new Decimal(1e100).times(Decimal.pow(1e5, player.dilation.rebuyables[i] - 23).times(1.05*player.dilation.rebuyables[i]-23));
  }
  return cost;
}
...
function updateDilationUpgradeCosts() {
    for (let i = 1; i <= 4; i++) {
      ...
      if (i === 2 && cost.gte(new Decimal("1e9999"))) document.getElementById("dil2cost").textContent = "Maxed out";
    }
...
function gameLoop(diff) {
    ...
    if (player.replicanti.unl && (diff > 5 || interval < 50 || player.timestudy.studies.includes(192))) {
        ...
        if (player.timestudy.studies.includes(192)) {
          gained = Decimal.pow(Math.E, current + Math.log((diff*est/10) * c + 1) / c);
        }
